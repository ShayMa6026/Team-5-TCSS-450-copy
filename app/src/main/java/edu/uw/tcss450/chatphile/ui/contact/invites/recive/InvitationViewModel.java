...
/**
 * View model for fragment_invitation
 *
 * @author Devin
 * @author Edwin
 * @author Yihan
...
public class InvitationViewModel extends AndroidViewModel {
    ...
    Instance fields
     */
    private MutableLiveData<List<Contact>> mInviteList;
    ...
    public MutableLiveData<List<Contact>> getmInviteList () {
        return this.mInviteList;
    }

    public void getIncomingContacts (final String jwt) {
        String url = getApplication().getResources().getString(R.string.base_url) + "contacts/received";
    ...
    public void acceptIncomingContacts (final String jwt, final int theFriendId) {
        String url = getApplication().getResources().getString(R.string.base_url) + "contacts/accept/" + theFriendId;
        Request request = new JsonObjectRequest(
                Request.Method.POST,
                url,
                null, //push token found in the JSONObject body
                this::handlePost, // we get a response but do nothing with it
                this::handleError) {
    ...
    private void handlePost (JSONObject result) {
        JSONObject response = result;

        if (response.has("success")) {
            Log.d("INVITE", "Accept Invite Success");
        }
    }

    private void handleGet (JSONObject result) {
        mInviteList.getValue().clear();
        MutableLiveData<List<Contact>> list = mInviteList;
